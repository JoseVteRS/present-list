 ...
 */
export const listGetAllByOwn = async (): Promise<ListsWithPresentsQueryResult> => {
    const currentUser = await auth()
    if (!currentUser) {
 ...
 */
export const listGetByIdShared = async (listId: string): Promise<[string | null, ListWithPresentsAndUser | null]> => {
    if (!listId) {
 ...
 * @returns 
 */
export const listGetById = async (listId: string): Promise<ListWithPresentsQueryResult> => {
 ...
 */
export const listUpdate = async (listId: string, values: z.infer<typeof ListEdit>) => {
    const currentUser = await auth()
    if (!currentUser) {
        console.error(LIST_ERROR.UNAUTHORIZED_USER)
        return ["Unauthorized. Please login", null]
